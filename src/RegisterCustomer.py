...
class RegisterCustomer(UserControl):
    def __init__(self, route):
        ...
        self.tf_CPF = TextField(
            label="CPF",
            expand=2,
            on_blur=self.check_cpf_cnpj,
            on_change=self.analyze_register_customer,
        )
        self.tf_phone = TextField(
            label="Telefone", expand=2, on_change=self.analyze_register_customer
        )
        self.tf_email = TextField(
            label="E-mail", expand=3, on_change=self.analyze_register_customer
        )
        self.tf_observ = TextField(
            label="Observação", expand=True, on_change=self.analyze_register_customer
        )
        ...
        self.dialog = AlertDialog(
            modal=True,
            title=Row(
                expand=True, controls=[Text("Novo endereço:", expand=True, width=800)]
            ),
            actions=[
                Column(
                    controls=[
    ...
    def ok_clicked(self, e):
        data = (
            str(self.tf_CPF.value),
            self.tf_adress.value,
            self.tf_city.value,
            self.tf_UF.value,
            self.tf_CEP.value,
        )
    ...
    def analyze_register_customer(self, e):
        ...
        if self.tf_name.value != "" and self.tf_CPF.value != "":
    ...
    def register_customer(self, e):
        ...
        if result != "success":
            Notification(
                self.page,
                f'Erro ao registrar o cliente: "{data_customer[0]}"! Erro: {result}',
                "red",
            ).show_message()
            return
    ...
    def update_customer(self, e):
        ...
        if result != "success":
            Notification(
                self.page,
                f'Erro ao atualizar o cliente: "{data_customer[1]}"! Erro: {result}',
                "red",
            ).show_message()
            return
        ...
        if result == "success":
            Notification(
                self.page,
                f'Cliente "{data_customer[1]}" atualizado com sucesso!',
                "green",
            ).show_message()
        else:
            Notification(
