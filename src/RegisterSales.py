...
class RegisterSales(Container):
    def __init__(self, route):
        ...
        self.text_description = Text(
            "Busque um Produto...", expand=True, style=TextThemeStyle.DISPLAY_SMALL
        )
        ...
        self.tf_date = TextField(
            border="none", expand=3, label="Data:", text_size=16, read_only=True
        )
        ...
        self.btn_close_sale = IconButton(
            ...
            icon_color="primary",
            icon_size=36,
            on_click=self.register_or_update_clicked,
        )
    ...
    def is_stock_available(self, id):
        ...
        if quantity > int(self.tf_stock.value):
            ...
            Notification(
                self.page,
                "Não há estoque suficiente para incluir o produto. Verifique a quantidade em estoque e os produtos já inclusos!",
                "red",
            ).show_message()
            self.update()
            return False
    ...
    def delete_product(self, e):
        if self.dt_products_sold.rows[e.control.data].cells[0].content.value != "Aut.":
            self.list_to_update_stock.append(
                [
                    self.products_in_table[e.control.data][1],
                    self.products_in_table[e.control.data][4],
                ]
            )
    ...
    def register_or_update_clicked(self, _):
        ...
        for row in range(len(self.dt_products_sold.rows)):
            data = [
                Validator.format_to_int(
                    self.dt_products_sold.rows[row].cells[1].content.value
                ),
                Validator.format_to_int(
                    self.dt_products_sold.rows[row].cells[4].content.value
                ),
                Validator.format_to_float(
                    self.dt_products_sold.rows[row].cells[5].content.value
                ),
                Validator.format_to_float(
                    self.dt_products_sold.rows[row].cells[6].content.value
                ),
                Validator.format_to_float(
                    self.dt_products_sold.rows[row].cells[7].content.value
                ),
            ]
    ...
    def register_sold_products(self, sale_id, products_data):
        ...
        if len(products_data) == len(result):
            ...
            return
        Notification(
            self.page, f"Erro ao cadastrar/atualizar o pedido: {result[-1]}", "red"
        ).show_message()
    ...
    def update_sale(self, id_sale, sale_data, products_data):
        ...
        if del_result == "success":
            ...
            Notification(
                self.page,
                f"Erro ao atualizar a venda (exclusão dos produtos vendidos): {del_result}",
                "red",
            ).show_message()
            return
    ...
    def update_stock(self, data):
        ...
        if len(result) == len(data):
            Notification(
                self.page,
                f"Pedido salvo com sucesso. {len(data)} produto(s) atualizado(s) no estoque!",
                "green",
            ).show_message()
        else:
            Notification(
