...
class Sales(Container):
    def __init__(self, route):
        ...
        self.btn_filter_previous_seven = IconButton(
            ...
            tooltip="Vendas dos Últimos Sete Dias",
            on_click=self.filter_buttons_clicked,
        )
        ...
        self.tf_customer = TextField(
            border="none",
            expand=9,
            label="Cliente:",
            value="ALEXANDER GRAHAN BELL DA SILVA SAURO",
            multiline=True,
            text_size=16,
            read_only=True,
            text_style=TextStyle(weight=FontWeight.BOLD),
    ...
    def fill_in_table_sales(self, data=None):
        ...
        if fulldata is None:
            Notification(
                self.page,
                "Erro ao carregar tabela de vendas. Reinicie o sistema.",
                "red",
            ).show_message()
            return
                            ...
                            controls=[
                                Row(
                                    ...
                                    controls=[
                                        IconButton(
                                            ...
                                            scale=0.9,
                                            on_click=self.edit_sale_clicked,
                                        ),
                                        Text(
                                            f"Pedido n. {data[0]}",
                                ...
                                Row(
                                    ...
                                    controls=[
                                        Text(f"Data: {data[2]}"),
                                        Text(
                                            f"Total: R${Validator.format_to_currency(data[3])}"
                                        ),
                                        Text(f"CPF: {data[5]}"),
                                    ],
                                ),
                                Text(
                                    f"Cliente: {data[4]}", text_align=TextAlign.CENTER
                                ),
                            ],
                        ),
                        ink=True,
                        on_click=lambda e: self.select_sale_clicked(
                            e.control.content.controls[0].controls[1].data
                        ),
                    ),
                ),
            )
                    ...
                    cells=[
                        ...
                        DataCell(
                            TextField(
                                border="none",
                                prefix_text="R$",
                                value=Validator.format_to_currency(data[5]),
                                read_only=True,
                            )
                        ),  # unit price
                        DataCell(
                            TextField(
                                border="none",
                                prefix_text="R$",
                                value=Validator.format_to_currency(data[6]),
                                read_only=True,
                            )
                        ),  # cost
                        DataCell(
                            TextField(
                                border="none",
                                prefix_text="R$",
                                value=Validator.format_to_currency(data[7]),
                                read_only=True,
                            )
                        ),  # total
                    ]
    ...
    def select_sale_clicked(self, idsale):
        with contextlib.suppress(Exception):
            ...
            if data_sale and data_sold_products:
                ...
                Notification(
                    self.page, "Erro ao carregar venda. Reinicie o sistema!", "red"
                ).show_message()
                self.btn_print.disabled = True
            self.update()
    ...
    def edit_sale_clicked(self, e):
        ...
        self.route.register_sales.sale_list_to_register = list(
            self.get_one_sale_from_db(e.control.data)
        )
        self.route.register_sales.products_in_table = self.get_all_sold_from_db(
            e.control.data
        )
    ...
    def delete_clicked(self, e):
        dialog = ConfirmDialog(
            self.delete_sale,
            "Por favor, confirme:",
            "Tem certeza que deseja apagar a venda? As informações não poderão ser recuperadas!",
        )
    ...
    def delete_sale(self, id_sale):
        ...
        if result == "success":
            ...
            Notification(
                self.page,
                f"Erro ao ecluir a venda (exclusão de produtos vendidos): {result}",
                "red",
            ).show_message()
            mydb.close()
            return
        ...
        dialog = ConfirmDialog(
            self.update_stock,
            "Deseja retornar os produtos ao estoque?",
            "Em caso positivo, os produtos desta venda serão novamente adicionados ao estoque.",
        )
    ...
    def update_stock(self, data_to_update):
        ...
        if len(result) == len(data_to_update):
            Notification(
                self.page,
                f"Sucesso! {len(result)} produto(s) com estoque atualizado!",
                "green",
            ).show_message()
        else:
            Notification(
                self.page,
                f"Não foi possível atualizar o estoque. Erro: {result[-1]}",
                "red",
            ).show_message()
    ...
    def get_products_data(self):
        ...
        for row in range(len(self.dt_products_sold.rows)):
            ...
            for col in range(len(self.dt_products_sold.columns)):
                (
                    text.append(
                        self.dt_products_sold.rows[row].cells[col].content.value
                    )
                    if col not in [6, 0]
                    else None
                )
    ...
    def get_header_data(self):
        return [
            self.route.config.company_name,
            self.route.config.company_adress,
            self.route.config.company_tel,
            self.route.config.company_email,
        ]
    ...
    def create_report_sales(self, e: FilePickerResultEvent):
        ...
        products_data.append(
            ["", "", "", "", "Total:", Validator.format_to_currency(total)]
        )
        ...
        if result == "success":
            Notification(
                self.route.page, f"Arquivo gerado com sucesso: {filename}", "green"
            ).show_message()
        else:
            Notification(
                self.route.page, f"Erro ao gerar o arquivo {filename}: {error}", "red"
            ).show_message()
